...
本文档用于梳理 `tiangong-lca-calculator` 当前的 provider link 逻辑，并基于线上最新 snapshot / 成功计算样本，说明 provider link 的核心问题与后续修改策略。

补充说明：

- 2026-03-28 当天，基于同一批线上 process 数据完成了四种 provider rule 的 replay 对比。
- 根据 replay 结果，`snapshot_builder` / worker 的默认 `provider_rule` 已切换为 `split_by_evidence_hybrid`。
...
截至 2026-03-28，本次分析所基于的最新 active snapshot：

- `snapshot_id = 12f9fbca-9a64-48a5-abbb-8333456fe929`
- `provider_matching_rule = strict_unique_provider`

这意味着：

- 当前线上 snapshot 在多 provider 情况下，本来就不会自动解析
- 所以 `matched_multi_unresolved` 高，不等于“自动解析逻辑失效”
- 更准确地说，是“当前生产配置选择了最保守策略”

但在 replay 验证完成后，当前代码默认值已经切到：

- `provider_rule = split_by_evidence_hybrid`

也就是说：

- 这组 `strict_unique_provider` 指标仍然是我们理解旧基线的关键样本
- 但它不再代表当前代码的默认目标行为
...
- `matched_multi_unresolved = 5814`
- `unmatched_no_provider = 4493`
- `a_input_edges_written = 1698`
- `a_write_pct = 14.14%`
- `provider_usable_pct = 14.14%`
- `any_provider_match_pct = 62.57%`

这里最重要的信号是：

- `provider_usable_pct == a_write_pct == 14.14%`

这说明当前 `A` 写入率几乎完全被 provider 解析能力限制住了。

不是“provider 已经选出来了但没有写进 A”，而是“能写进 A 的边，基本就是那些唯一 provider 的边”。

### 3.3 当前 active snapshot 与最新成功 solve-backed snapshot 几乎一致

最新成功 solve 对应 snapshot：

- `snapshot_id = c1c84a89-458c-4bca-807e-fbc8cfacff20`

它在 provider / A 指标上和最新 active snapshot 基本一致：
...
说明当前最新 active snapshot 在 provider link 层面并没有出现结构性改善。

---

## 4. 对 multi-provider unresolved 的进一步分析

下面的分析基于最新 active snapshot `12f9fbca-9a64-48a5-abbb-8333456fe929`，并复现了当前代码中的打分逻辑。
...
当前 provider link 的核心矛盾，不是“数据库里没有 provider”，也不是“代码根本不会自动选”，而是：

- 旧基线样本里，生产默认策略停留在 `strict_unique_provider`
- 当前代码默认策略已经切到 `split_by_evidence_hybrid`
- 候选 provider 集合按 `flow_id` 聚合得太粗
- 自动解析又只靠 `model_id + location + year`
- 导致大量已经有候选的 input edge 无法被唯一决策

因此，当前最值得优先推进的不是继续讨论 solve 端，而是把下面这条链路打透：

`input edge -> provider candidate set -> provider evidence -> provider resolution -> A write`

只要这条链路的可解释性和转化率上来了，`A` 写入率、snapshot 网络厚度和最终结果稳定性才会一起改善。
