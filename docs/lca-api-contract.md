...
- 计算入口是异步任务；legacy 路径使用 `lca_jobs` + `pgmq`，统一队列路径使用 `worker_jobs(worker_queue=solver)`。前端不直连队列。
...
- `lca_jobs`: 异步作业主表。
- `worker_jobs`: 统一 worker 生命周期表；solver 队列任务使用 `worker_queue=solver`，用于服务端任务中心、operator 查询和 lease fencing。
...
legacy `lca_jobs.job_type` 与 worker payload `type` 必须一致。`worker_jobs` 路径使用 `job_kind` 表达统一队列类型，并在 calculator 内部映射回同一组 legacy payload `type`，从而复用既有求解和 artifact 持久化逻辑。
...
solver worker 可通过 `SOLVER_QUEUE_BACKEND=worker-jobs` 或 `--queue-backend worker-jobs` 切到 `public.worker_jobs` claim 模式。该模式只领取 `worker_queue=solver` 的 calculator jobs：
...
4. worker 使用 `worker_claim_jobs('solver', ...)` claim、heartbeat，并通过 `worker_record_job_result(...)` 写终态；同时继续维护 `lca_jobs`、`lca_results`、`lca_result_cache`。
