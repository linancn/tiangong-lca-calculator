...
从 input exchange 中确定被需求的 product/reference flow `f`。provider candidates 必须能够提供同一 `f`。

如果没有可用 provider，calculator 不应构造虚拟 provider。该 exchange 应进入 provider-link diagnostics，由数据修复、补充 provider，或显式 market process 建模解决。
