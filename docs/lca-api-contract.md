...
snapshot coverage diagnostics 会暴露 provider decision 统计，用于解释 snapshot 构建阶段的 provider linking 行为。当前包括 resolution strategy、unresolved reason、geography tier、volume fallback-to-one，以及 supply-region anchor 来源统计。这些字段属于诊断信息，不改变 job payload、状态机或 result artifact 主契约。
