...
if command -v uv >/dev/null 2>&1; then
  exec uv run --project "$PROJECT_DIR" bw25-request-scope-validate "$@"
fi
...
if [ ! -x "$VENV_DIR/bin/bw25-request-scope-validate" ]; then
  "$PYTHON_BIN" -m venv "$VENV_DIR"
  "$VENV_DIR/bin/pip" install --upgrade pip
  "$VENV_DIR/bin/pip" install -e "$PROJECT_DIR"
fi

exec "$VENV_DIR/bin/bw25-request-scope-validate" "$@"
